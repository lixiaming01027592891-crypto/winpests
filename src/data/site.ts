...
export const services = [
  {
    ...
    signals: ["門框或踢腳板敲起來發空", "牆角、窗邊出現脫落翅膀", "木作旁反覆出現細粉或泥線"],
    inspection: ["木作受損與相鄰結構", "漏水、壁癌與潮濕來源", "地板、門框和管線交接處"],
    treatment: ["未拆結構評估鑽孔灌注", "外露木作做噴藥與縫隙處理", "裝潢前依施工階段安排預防"],
    pricing: ["施作面積與受損範圍", "木作是否已拆開", "樓層、動線與保固範圍"],
  ...
  },
  {
    ...
    signals: ["夜間聽見天花或夾層聲音", "牆邊出現鼠糞、咬痕或油污", "食物與垃圾袋反覆被翻動"],
    inspection: ["管線孔、門縫與排水周邊", "牆邊固定行走路徑", "食物、水源和垃圾動線"],
    treatment: ["依動線安排餌站或捕捉", "確認活動下降後再做封孔", "需要時安排回訪追蹤"],
    pricing: ["活動範圍與佈點數量", "封孔位置和施工難度", "是否需要多次追蹤"],
  ...
  },
  {
    ...
    signals: ["同一區域反覆看到蟑螂或螞蟻", "人或寵物身上出現不明叮咬", "排水、廚房或儲藏區蟲量增加"],
    inspection: ["水源、食物與藏匿縫隙", "寵物活動區和布面家具", "排水口、櫥櫃與管線周邊"],
    treatment: ["依蟲種搭配餌劑與縫隙處理", "需要噴灑時先安排離場與通風", "店面可配合打烊後施工"],
    pricing: ["蟲種、面積和發生範圍", "住家或營業場所時段", "是否需要後續追蹤"],
  ...
  },
  {
    ...
    signals: ["工區積水後蚊蟲明顯增加", "公共區域需要定期環境處理", "施工或營運時段不能長時間中斷"],
    inspection: ["積水點、排水與雜物堆放", "人車主要動線與封閉範圍", "需避開的設備、課程或作業"],
    treatment: ["依現場範圍安排局部噴灑", "配合作業空檔分區處理", "需要時提供施工說明"],
    pricing: ["面積、樓層與處理區域", "進場時段和分區次數", "是否需要文件與後續巡查"],
...
export const cases = [
  {
    ...
    signal: "積水後蚊蟲變多，工區仍有人車持續進出。",
    inspect: "積水點、排水路徑、雜物區與主要施工動線。",
    direction: "依工序分區處理，並把容易反覆積水的位置一起標出。",
  },
  {
    slug: "home-termite",
    title: "透天木作處理",
    ...
    signal: "門框發空、木作附近出現翅膀、細粉或可疑泥線。",
    inspect: "相鄰木作、牆面潮濕、漏水處與地板交接。",
    direction: "先確認範圍，再評估灌注、外露木作處理與保固內容。",
  },
  {
    ...
    signal: "柱邊有鼠糞或油污，夜間監視器反覆拍到活動。",
    inspect: "管線孔、車道交接、垃圾區與固定牆邊動線。",
    direction: "先佈點確認活動，再處理入口與垃圾動線。",
  },
  {
    slug: "night-shop",
    title: "打烊後店面處理",
    ...
    signal: "廚房、排水或儲藏區在打烊後仍持續見蟲。",
    inspect: "水源、食材收納、設備縫隙與後場排水。",
    direction: "依蟲種搭配餌劑與縫隙處理，並安排不影響營業的時段。",
  },
  {
    slug: "school-hall",
    title: "校園走廊環境處理",
    ...
    signal: "走廊、鞋櫃或積水處在特定時段蟲量增加。",
    inspect: "師生動線、排水、植栽與容易積水的公共區域。",
    direction: "利用非上課時段分區處理，並清楚標示等待時間。",
  },
  {
    slug: "office-night",
    title: "辦公茶水間勘查",
    ...
    signal: "茶水間、影印區或踢腳附近偶爾出現蟑螂。",
    inspect: "水槽、櫥櫃、管線孔與電器後方的溫暖縫隙。",
    direction: "下班後勘查與佈點，減少對辦公時段的干擾。",
  },
] as const;
